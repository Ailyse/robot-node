function findByIdWithModel(id, PersistentModel) {
  return new Promise((resolve, reject) => {
    PersistentModel.findById(id, function (err, entity) {
...
function findByCriteriaWithModel(criteria, PersistentModel) {
  return new Promise((resolve, reject) => {
    PersistentModel.find(criteria, function (err, entities) {
...
function findFirstDocumentWithModel(PersistentModel) {
  return new Promise((resolve, reject) => {
    PersistentModel.findOne(function (err, entity) {
      if (err) reject({ error: "can not find", cause: err });
      else resolve(entity);
    });
  });
...
function updateOneWithModel(
  ...
  return new Promise((resolve, reject) => {
    ...
    PersistentModel.updateOne(
      filter,
      newValueOfEntityToUpdate,
      function (err, opResultObject) {
        console.log(
          "opResultObject of updateOne=" + JSON.stringify(opResultObject)
        );
...
function deleteOneWithModel(idOfEntityToDelete, PersistentModel) {
  return new Promise((resolve, reject) => {
    ...
    PersistentModel.deleteOne(filter, function (err, opResultObject) {
      console.log(
        "opResultObject of deleteOne=" + JSON.stringify(opResultObject)
      );
      if (err) reject({ error: "cannot delete ", cause: err });
      else if (opResultObject.deletedCount == 1)
        resolve({ deletedId: idOfEntityToDelete });
      else
        reject({
          error: "NOT_FOUND",
...
module.exports.findFirstDocumentWithModel = findFirstDocumentWithModel;
